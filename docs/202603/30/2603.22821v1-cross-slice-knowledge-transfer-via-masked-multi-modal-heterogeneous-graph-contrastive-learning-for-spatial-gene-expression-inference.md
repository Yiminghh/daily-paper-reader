...
tags: ["query:molrna"]
score: 7.0
evidence: 空间基因表达的异构图对比学习
tldr: 空间转录组学（ST）成本高昂，从病理图像预测ST是有效替代方案，但现有方法难以捕捉跨切片的复杂空间关系。本文提出SpaHGC，一种基于多模态异构图的模型，通过病理基础模型提取图像嵌入，捕捉切片内和切片间的点对关系。结合掩码图对比学习，SpaHGC实现了从参考切片到目标切片的知识迁移，显著提升了基因表达预测的准确性。
...
conclusion: 该研究证明了跨切片知识迁移在提升空间基因表达预测准确性方面的有效性，并展示了其在癌症研究中的生物学应用潜力。
---

## 摘要
虽然空间转录组学（ST）增进了我们对组织背景下基因表达的理解，但其高昂的实验成本限制了其大规模应用。从病理图像预测 ST 是一种极具前景且具有成本效益的替代方案，但现有方法难以捕捉复杂的跨切片空间关系。为了应对这一挑战，我们提出了 SpaHGC，这是一种基于多模态异构图的模型，能够从组织学图像中捕捉切片内和切片间的位点（spot）关系。它整合了目标切片内的局部空间上下文，以及由病理基础模型提取的图像嵌入所计算出的跨切片相似性。这些嵌入实现了切片间的知识迁移，SpaHGC 进一步结合了掩码图对比学习以增强特征表示，并将空间基因表达知识从参考切片迁移到目标切片，使其能够建模复杂的空间依赖性并显著提高预测准确性。我们在来自不同平台、组织和癌症亚型的七个匹配的组织学-ST 数据集上进行了全面的基准测试。结果表明，SpaHGC 在所有评估指标上均显著优于现有的九种最先进方法。此外，预测结果在多个癌症相关通路中显著富集，从而突显了其强大的生物学相关性和应用潜力。
