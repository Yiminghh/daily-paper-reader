...
title_zh: 自动调优 T-PaiNN：通过从经典到量子的迁移学习实现数据高效的 GNN 原子间势开发
...
tags: ["query:molrna"]
score: 8.0
evidence: 基于GNN的原子间势能和分子表示学习
tldr: 本研究针对机器学习原子间势能模型（MLIP）对昂贵量子力学数据依赖性强的问题，提出了 T-PaiNN 迁移学习框架。该方法通过在廉价的大规模经典力场数据上预训练 PaiNN 架构，随后利用少量 DFT 数据进行微调，实现了从经典到量子的知识迁移。实验表明，该方法在气相分子和液态水系统中均显著提升了数据效率，大幅降低了预测误差并加快了收敛速度，为复杂化学系统的高效建模提供了新途径。
selection_source: fresh_fetch
motivation: 机器学习原子间势能模型通常需要海量昂贵的量子力学训练数据，限制了其在实际科研中的广泛部署。
method: 提出 T-PaiNN 框架，通过在经典分子动力学模拟生成的廉价数据上预训练，再用少量 DFT 数据进行微调（自动调优）。
result: 在 QM9 和液态水测试中，该方法在低数据量下将误差降低了最高 25 倍，并显著改善了密度和扩散等物理性质的预测。
conclusion: 从经典力场到量子力学的迁移学习是开发高精度、数据高效型 GNN 原子间势能模型的一种实用且高效的策略。
---

## 摘要
机器学习原子间势 (MLIP)，特别是基于图神经网络 (GNN) 的模型，为在显著降低计算成本的同时实现接近密度泛函理论 (DFT) 的精度提供了一条极具前景的途径。然而，它们的实际应用往往受到所需的大量昂贵量子力学训练数据的限制。在这项工作中，我们引入了一种迁移学习框架 Transfer-PaiNN (T-PaiNN)，通过利用廉价的经典力场数据，显著提高了 GNN-MLIP 的数据效率。该方法包括在经典分子模拟生成的大规模数据集上预训练 PaiNN MLIP 架构，随后使用相对较小的 DFT 数据集进行微调（称为自动调优）。我们在气相分子系统（QM9 数据集）和凝聚相液态水上证明了自动调优 T-PaiNN 的有效性。在所有案例中，T-PaiNN 的表现均显著优于仅在 DFT 数据上训练的模型，在加速训练收敛的同时，实现了平均绝对误差的数量级降低。例如，使用 QM9 数据集，在低数据量情况下观察到误差降低了多达 25 倍，而液态水模拟显示出对能量、力和实验相关属性（如密度和扩散）的预测有所改善。这些提升源于模型能够从广泛的经典采样中学习势能面的通用特征，随后将其细化至量子精度。总之，这项工作确立了从经典力场进行迁移学习是一种开发高精度、数据高效的 GNN 原子间势的实用且计算高效的策略，从而使 MLIP 能够更广泛地应用于复杂的化学系统。
