...
title_zh: CellFluxRL：通过强化学习实现的生物约束虚拟细胞建模
...
## 摘要
使用生成模型构建虚拟细胞以在计算机内（in silico）模拟细胞行为，正成为加速药物研发的一种极具前景的范式。然而，先前的基于图像的生成方法可能会产生不合理的细胞图像，违反基本的物理和生物约束。为了解决这一问题，我们提出利用强化学习（RL）对虚拟细胞模型进行后训练，并采用具有生物学意义的评估器作为奖励函数。我们设计了涵盖生物功能、结构有效性和形态正确性三个类别的七种奖励，并对最先进的 CellFlux 模型进行了优化，从而得到 CellFluxRL。CellFluxRL 在所有奖励指标上均一致优于 CellFlux，且通过测试时缩放（test-time scaling）进一步提升了性能。总之，我们的研究结果展示了一个通过强化学习强制执行物理约束的虚拟细胞建模框架，实现了从“视觉真实”生成向“生物学意义”生成的跨越。
