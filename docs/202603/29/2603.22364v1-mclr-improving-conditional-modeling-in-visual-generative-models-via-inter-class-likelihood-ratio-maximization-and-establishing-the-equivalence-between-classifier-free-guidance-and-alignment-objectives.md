...
title_zh: MCLR：通过类间似然比最大化改进视觉生成模型中的条件建模，并建立无分类器指导与对齐目标之间的等价性
authors: "Xiang Li, Yixuan Jia, Xiao Li, Jeffrey A. Fessler, Rongrong Wang, Qing Qu"
date: 2026-03-23
pdf: "https://arxiv.org/pdf/2603.22364v1"
tags: ["query:molrna"]
score: 6.0
evidence: 改进扩散生成模型中的条件建模
tldr: 扩散模型通常依赖推理时的CFG启发式方法来提升生成质量。本文指出标准训练目标在类别分离度上存在不足，提出了MCLR对齐目标，通过在训练中最大化类间似然比，使模型在不使用CFG的情况下也能达到类似的生成效果。此外，研究证明了CFG与加权MCLR目标在理论上的等价性，为CFG提供了机械论解释。
selection_source: fresh_fetch
motivation: 旨在解决扩散模型对推理时CFG引导的过度依赖，并探索如何通过改进训练目标实现同等效果。
method: 提出MCLR对齐目标，通过在训练过程中显式最大化类间似然比来增强模型的条件建模能力。
result: 使用MCLR微调的模型在无需推理引导的情况下，取得了与CFG相当的定性和定量生成增益。
conclusion: 证明了CFG引导的得分函数是加权MCLR目标的优化解，从而在理论上建立了CFG与对齐目标之间的等价性。
---

## 摘要
扩散模型在生成建模中取得了最先进的性能，但其成功往往在很大程度上依赖于无分类器指导（CFG），这是一种在推理阶段修改采样轨迹的启发式方法。从理论角度来看，使用标准去噪得分匹配（DSM）训练的扩散模型预期能够恢复目标数据分布，这引发了一个问题：为什么在实践中推理阶段的指导是必要的。在这项工作中，我们探讨是否可以以一种原则性的方式修改 DSM 训练目标，使得在没有推理阶段指导的情况下，标准反向时间采样也能产生与 CFG 相当的效果。我们发现类间分离不足是标准扩散模型的一个关键局限。为了解决这一问题，我们提出了 MCLR，这是一种原则性的对齐目标，在训练过程中显式地最大化类间似然比。使用 MCLR 微调的模型在标准采样下表现出类似于 CFG 的改进，在不需要推理阶段指导的情况下实现了相当的定性和定量提升。除了实证效益外，我们还提供了一个理论结果，证明 CFG 指导的得分正是加权 MCLR 目标的最佳解。这建立了无分类器指导与基于对齐的目标之间的形式等价性，为 CFG 提供了一种机制解释。
