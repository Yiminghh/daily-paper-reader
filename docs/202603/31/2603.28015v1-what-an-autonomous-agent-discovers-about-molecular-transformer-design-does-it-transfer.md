...
title_zh: 自主智能体在分子 Transformer 设计中的发现：这些发现是否可迁移？
...
tldr: 本研究探讨了分子和蛋白质序列是否需要特定的Transformer架构设计。通过在SMILES、蛋白质和英文文本上进行3106次自主架构搜索实验，发现对于SMILES，调整超参数比搜索新架构更有效，而NLP则更依赖架构改进。尽管搜索出的架构在不同领域表现出差异，但这些创新在跨领域迁移时性能损失极小，表明架构差异更多源于搜索路径而非生物学特性。
selection_source: fresh_fetch
motivation: 探究广泛应用于分子和蛋白质建模的Transformer架构是否需要针对生物序列进行专门的定制化设计。
method: 利用自主智能体在SMILES、蛋白质和自然语言三种序列上进行大规模架构搜索，并对比超参数优化与架构改进的效果。
result: 发现SMILES建模中超参数调优优于架构搜索，且不同领域发现的架构创新具有极强的跨领域通用性。
conclusion: 分子建模中架构设计的差异主要源于搜索过程的随机性而非领域特有的生物学需求，建议优先进行超参数调优。
---

## 摘要
用于类药分子和蛋白质的深度学习模型绝大多数沿用了为自然语言设计的 Transformer 架构，然而分子序列是否能从不同的设计中获益尚未得到系统性测试。我们通过智能体在三种序列类型（SMILES、蛋白质和作为对照的英文文本）上部署了自主架构搜索，在单块 GPU 上运行了 3,106 次实验。研究发现，对于 SMILES，架构搜索适得其反：仅调整学习率和调度策略的效果就优于完整的架构搜索 (p = 0.001)。对于自然语言，架构变化驱动了 81% 的性能提升 (p = 0.009)。蛋白质的表现则介于两者之间。令人惊讶的是，尽管智能体为每个领域发现了截然不同的架构 (p = 0.004)，但每一项创新在所有三个领域之间迁移时的性能下降均小于 1%，这表明这些差异反映的是搜索路径的依赖性，而非根本的生物学需求。我们发布了一个决策框架和开源工具包，旨在帮助分子建模团队在自主架构搜索与简单的超参数微调之间做出选择。
