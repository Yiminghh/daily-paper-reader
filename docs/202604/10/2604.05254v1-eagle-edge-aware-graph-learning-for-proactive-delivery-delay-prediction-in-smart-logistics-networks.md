...
title_zh: EAGLE：用于智慧物流网络主动式配送延迟预测的边缘感知图学习
...
tldr: 针对现代物流网络中交付延迟预测滞后的问题，本文提出EAGLE框架。该框架结合了轻量级Transformer补丁编码器和边缘感知图注意力网络（E-GAT），同时建模订单流的时间动态和物流枢纽间的空间依赖关系。在DataCo数据集上的实验表明，EAGLE在预测准确率和训练稳定性方面均优于现有方法，为主动式供应链风险管理提供了有效方案。
selection_source: fresh_fetch
motivation: 现有物流延迟预测方法往往忽略了供应链网络中的拓扑结构和空间依赖性，导致预测缺乏前瞻性。
method: 提出一种结合Transformer补丁编码器与边缘感知图注意力网络（E-GAT）的混合深度学习框架，通过多任务学习优化时空特征。
result: 在真实数据集上达到0.8762的F1分数和0.9773的AUC-ROC，且训练稳定性比消融变体提升了3.8倍。
conclusion: EAGLE框架成功实现了对物流延迟的高精度、高稳定性预测，证明了结合时空建模在主动式供应链管理中的优越性。
---

## 摘要
现代物流网络在每个仓库节点和运输路线上都会产生丰富的运营数据流——从订单时间戳、路由记录到货运清单——然而，配送延迟预测目前仍主要处于被动响应阶段。现有的预测方法通常将该问题视为表格分类任务（忽略了网络拓扑结构），或者视为时间序列异常检测任务（忽视了供应链图的空间依赖性）。为了弥补这一差距，我们提出了一种用于主动式供应链风险管理的混合深度学习框架。该方法通过轻量级 Transformer patch 编码器对时间维度的订单流动态进行建模，并通过边缘感知图注意力网络（E-GAT）对枢纽间的关系依赖进行建模，并利用多任务学习目标进行优化。在真实的 DataCo 智慧供应链数据集上的评估结果显示，我们的框架相比基准方法取得了持续的改进，F1 分数达到 0.8762，AUC-ROC 达到 0.9773。在四个独立的随机种子实验中，该框架的跨种子 F1 标准差仅为 0.0089——比表现最好的消融变体提升了 3.8 倍——在所有评估模型中实现了预测准确性与训练稳定性之间的最佳平衡。
