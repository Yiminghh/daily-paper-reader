...
motivation: 量子算法设计过程复杂且反直觉，传统人工方法难以在近期量子设备上实现资源消耗与计算精度的最优平衡。
method: 采用基于大语言模型驱动的分布式进化程序合成平台Hive，自动搜索并优化用于求解分子基态的量子启发式算法。
result: 成功为LiH、H2O和F2分子发现了资源消耗显著低于现有最先进水平的高效量子电路，并在真实量子硬件上达到了化学精度。
conclusion: AI驱动的自动化发现方法不仅能有效优化近期量子算法，还可扩展至其他领域及未来的容错量子计算设计。
---

## 摘要
量子算法的设计是一项复杂且违背直觉的任务，这使其成为人工智能驱动算法发现的理想候选领域。为此，我们采用了 Hive，这是一个用于程序合成的 AI 平台，它利用大语言模型驱动高度分布式的演化过程来发现新算法。我们专注于量子化学中的基态问题，并发现了能够解决 LiH、H2O 和 F2 分子基态问题的高效量子启发式算法，与最先进的近期量子算法相比，这些算法在量子资源消耗方面表现出显著降低。此外，我们对发现的算法进行了可解释性研究，并确定了促成效率提升的关键函数。最后，我们在 Quantinuum System Model H2 量子计算机上对 Hive 发现的电路进行了基准测试，并确定了达到化学精度所需的最低系统要求。我们预见这种新颖的量子算法发现方法不仅适用于化学以外的其他领域，也适用于设计容错量子计算机的量子算法。
