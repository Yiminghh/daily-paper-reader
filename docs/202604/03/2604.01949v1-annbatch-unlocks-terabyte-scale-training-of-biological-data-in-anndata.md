...
score: 6.0
evidence: 大规模生物数据训练
tldr: 随着生物数据集规模突破内存限制，数据加载成为AI训练瓶颈。本文推出annbatch，这是一款原生支持anndata格式的微批次加载器，支持直接在磁盘存储的数据集上进行核外训练。它在单细胞、显微镜和全基因组测序等任务中将吞吐量提升了一个数量级，将训练时间从数天缩短至数小时，且完美兼容scverse生态，为大规模生物AI提供了高效的基础设施。
selection_source: fresh_fetch
motivation: 生物数据集规模日益庞大且格式复杂，导致数据访问而非模型计算成为机器学习训练的主要瓶颈。
method: 开发了原生于anndata的微批次加载器annbatch，支持对磁盘备份的数据集进行高效的核外训练。
result: 在多项生物学基准测试中，annbatch将加载吞吐量提升了高达10倍，显著缩短了模型训练周期。
conclusion: annbatch为可扩展的生物AI提供了实用的数据加载方案，在保留标准数据格式的同时实现了TB级数据的训练。
---
