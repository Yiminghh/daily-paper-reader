...
motivation: 现有的图嵌入方法在处理包含数亿至数十亿条边的超大规模图时，面临单机内存不足和计算效率低下的严峻挑战。
method: 提出基于MPI的分布式内存框架NOMAD，通过实现LINE算法模型并优化分布式环境下的通信开销与数据处理权衡来提升性能。
result: 在NERSC Perlmutter集群上，NOMAD比多线程LINE/node2vec快10-100倍，比分布式PBG快35-76倍，且嵌入质量与主流方法相当。
