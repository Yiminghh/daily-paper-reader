...
tldr: 本文提出了一种广义扩散模型框架，将传统的独立噪声注入替换为包含相互作用结构的马尔可夫链蒙特卡罗（MCMC）动力学。通过在扩散过程中引入Ising耦合，该方法能有效利用目标系统的空间相关性。该框架可无缝映射到由概率位（p-bits）构成的概率计算机上，在采样吞吐量和能效上相比GPU具有数量级优势，为生成式建模提供了结合结构化采样的新路径。
selection_source: fresh_fetch
motivation: 传统扩散模型主要依赖独立噪声采样，限制了其在处理具有复杂空间相关性的物理系统时的效率和准确性。
method: 通过在扩散动力学中显式引入Ising耦合，将独立扩散推广为包含MCMC动力学的相关扩散，并利用概率硬件进行加速实现。
result: 在2D铁磁Ising模型和3D自旋玻璃模型上的实验表明，相关扩散生成的样本比独立扩散更接近MCMC参考分布。
conclusion: 该研究证明了结合结构化概率采样的相关扩散算法能显著提升生成建模性能，并展示了概率计算机在加速新型扩散算法方面的巨大潜力。
---
