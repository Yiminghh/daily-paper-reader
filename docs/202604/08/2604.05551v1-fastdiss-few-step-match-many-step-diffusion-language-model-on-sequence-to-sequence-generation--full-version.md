...
tldr: 针对连续扩散语言模型在少步采样（快速推理）时自调节失效导致的误差累积问题，本文提出了FastDiSS框架。该框架通过在训练中引入扰动自调节信号来模拟推理噪声，并结合令牌级噪声感知机制优化训练。实验证明，FastDiSS在保持生成质量的同时，推理速度提升达400倍，且在条件生成任务中表现优异。
selection_source: fresh_fetch
motivation: 扩散语言模型在少步采样时，自调节信号的不准确会导致误差在去噪步骤中不断累积，严重影响生成质量。
method: 提出一种新的训练框架，通过扰动自调节信号以匹配推理噪声来增强鲁棒性，并引入令牌级噪声感知机制以防止训练饱和。
result: 在多个条件生成基准测试中，该方法在推理速度提升高达400倍的情况下，性能超越了标准扩散模型，并与其他单步扩散框架具有竞争力。
conclusion: FastDiSS 有效解决了扩散模型在快速推理中的误差累积问题，实现了高效且高质量的序列到序列生成。
---
